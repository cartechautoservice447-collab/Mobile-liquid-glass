(() => {
  ...
  function showPomodoro() {
    ...
    qsa(overlay, '[data-minutes]').forEach((button) => button.addEventListener('click', () => { timerSeconds = Number(button.dataset.minutes)*60; running=false; if(timer)clearInterval(timer); timer=null; qsa(overlay,'[data-minutes]').forEach((b)=>b.classList.remove('active')); button.classList.add('active'); updateTimer(display); }));
    play.addEventListener('click', () => { running=!running; play.textContent=running?'Ⅱ':'▶'; if(running) timer=setInterval(()=>{ if(timerSeconds<=0){running=false;clearInterval(timer);timer=null;play.textContent='▶';}else timerSeconds-=1;updateTimer(display);},1000); else if(timer){clearInterval(timer);timer=null;} });
  }
  ...
  function showMore(){const overlay=shell('Quick Access','Liquid Glass Studio',`<div class="reference-more-actions"><button data-open="notes">Notes</button><button data-open="pomodoro">Pomodoro Timer</button><button data-open="overview">Overview</button><button data-open="settings">Settings</button><button data-open="courses">Course Folders</button><button data-open="add">Add New Course</button></div>`,'more');qsa(overlay,'[data-open]').forEach((b)=>b.addEventListener('click',()=>{const key=b.dataset.open;if(key==='notes')showNotes();else if(key==='pomodoro')showPomodoro();else if(key==='overview')showOverview();else if(key==='settings')showSettings();else if(key==='add')showAddCourse();else showCourses();}));}

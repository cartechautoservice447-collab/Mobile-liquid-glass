...
export default function PomodoroModal({ close }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Keep the timer state continuous so the ring receives every fractional
      // clock update. The visible MM:SS display still rounds with Math.ceil().
      setRemaining(next);
    ...
    // Start the first clock sample immediately. This makes the ring begin
    // moving from the exact moment Start is pressed.
    tick();

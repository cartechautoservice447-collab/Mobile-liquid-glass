...
function readLiquidSpring(){
 const root=document.documentElement;
 const stiffness=Number.parseFloat(root.style.getPropertyValue('--liquid-bounce'));
 const damping=Number.parseFloat(root.style.getPropertyValue('--liquid-bounce-damping'));
 const gel=Number.parseFloat(root.style.getPropertyValue('--liquid-gel'));
 return {
  type:'spring',
  stiffness:Number.isFinite(stiffness)?stiffness:200,
  damping:Number.isFinite(damping)?damping:24,
  mass:0.6+(Number.isFinite(gel)?gel:0.55)/140,
 };
}

export default function CollectionWorkspace({collection,addNote,onBack,onSaveNote,onDeleteNote,message}){
 const[view,setView]=useState('notes');const[draftNote,setDraftNote]=useState(null);const[hiddenNoteIds,setHiddenNoteIds]=useState(()=>new Set());const[glassSpring,setGlassSpring]=useState(readLiquidSpring);
 ...
 useEffect(()=>{const sync=()=>setGlassSpring(readLiquidSpring());sync();window.addEventListener('glass-settings-changed',sync);return()=>window.removeEventListener('glass-settings-changed',sync);},[]);

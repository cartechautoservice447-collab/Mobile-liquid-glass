...
export default function CollectionWorkspace({ course, collection, newNoteName, setNewNoteName, addNote, onBack, openNote, message, onDeleteNote, onToggleFavorite }) {
  ...
  const saveDraft = () => {
    if (!draft) return;
    // The parent remains the data boundary; return through the existing note route after save.
    openNote(draft.id);
  };
            ...
            <div className="collection-note-list">
              {visibleNotes.length === 0 ? <div className="collection-empty"><FileText size={24} /><strong>{panel === 'favorites' ? 'No favorite notes' : 'No notes yet'}</strong><span>Create a note or change your search.</span></div> : visibleNotes.map((note, index) => (
                <article className="collection-note-card" key={note.id} onClick={() => selectNote(note)}>
                  ...
                  <div className="collection-note-copy"><strong>{note.title || 'Untitled note'}</strong><span>{snippet(note.content) || 'Empty note'}</span></div>

...
export default function CourseWorkspace({ course, onBack, onCourses, onOpenCollection, onOpenNote, onStudySession, onOverview }) {
  ...
  const tools = [
    ...
    { title: 'Collections', text: `${collections} ${collections === 1 ? 'collection' : 'collections'} in this course.`, icon: <FolderOpen size={21} />, onClick: () => course.collections[0] && openCollection(course.collections[0].id) },
    { title: 'All Notes', text: `Open all ${noteCount} ${noteCount === 1 ? 'note' : 'notes'} stored in this course.`, icon: <FileText size={21} />, onClick: onOpenNote ? () => {
      const first = course.collections.find((collection) => collection.notes.length)?.notes[0];
      const firstCollection = course.collections.find((collection) => collection.notes.length);
      if (first && firstCollection) openNote(firstCollection.id, first.id);
    } : undefined },

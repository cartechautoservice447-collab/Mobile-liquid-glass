    ...
    <main className="screen feature-screen">
      <section className="full-glass-panel course-workspace-panel">
        ...
        <section className="course-workspace-actions">
          <button className="glass-card course-tool-card" onClick={onStudySession}>
            <span className="action-icon"><Play size={21} /></span>
            <span><strong>Study Session</strong><small>Start studying this course and track the session.</small></span>
            <span className="action-arrow">›</span>
          </button>
          <button className="glass-card course-tool-card" onClick={onPomodoro}>
            <span className="action-icon"><Timer size={21} /></span>
            <span><strong>Pomodoro</strong><small>Focus with timed work and break sessions.</small></span>
            <span className="action-arrow">›</span>
          </button>
          <button className="glass-card course-tool-card" onClick={onCollections}>
            <span className="action-icon"><FolderOpen size={21} /></span>
            <span><strong>Collections</strong><small>Organize the notes inside this course.</small></span>
            <span className="action-arrow">›</span>
          </button>
          <button className="glass-card course-tool-card" onClick={onNotes}>
            <span className="action-icon"><FileText size={21} /></span>
            <span><strong>All Notes</strong><small>Open and continue writing course notes.</small></span>
            <span className="action-arrow">›</span>
          </button>
          <button className="glass-card course-tool-card" onClick={onOverview}>
            <span className="action-icon"><BookOpen size={21} /></span>
            <span><strong>Course Overview</strong><small>See course activity, notes and progress.</small></span>
            <span className="action-arrow">›</span>
          </button>
        </section>

        <section className="glass-inner course-workspace-summary">
          <div><span className="section-label">Course progress</span><strong>{course.progress || 0}% complete</strong></div>
          <div className="progress-track"><span style={{ width: `${Math.min(100, Math.max(0, course.progress || 0))}%` }} /></div>
          <p>{totalNotes} notes across your mobile workspace.</p>
        </section>
      </section>
    </main>
  );

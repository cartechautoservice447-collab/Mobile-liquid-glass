...
const COURSE_ACCENTS = [
  { name: 'sky', value: '#72d7ff' },
  { name: 'violet', value: '#bd86ff' },
  { name: 'amber', value: '#ffd166' },
  { name: 'emerald', value: '#67e8b1' },
  { name: 'rose', value: '#ff88a8' },
  { name: 'cyan', value: '#65e6ff' },
];
...
function CourseCreateModal({ close, name, description, color, setName, setDescription, setColor, submit }) {
  return <div className="modal-backdrop" onClick={close}><section className="glass-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={close} aria-label="Close"><X size={18} /></button><span className="modal-symbol"><Plus size={22} /></span><h2>New course</h2><p>Fill in your course details, then confirm to create the course card.</p><form onSubmit={submit} className="course-create-form"><label><span>Course name</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Organic Chemistry II" autoFocus required /></label><label><span>Description</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="What's this course about?" rows={3} /></label><div className="course-form-row"><span className="course-form-label">Accent color</span><div className="accent-options">{COURSE_ACCENTS.map((accent) => <button key={accent.name} type="button" aria-label={accent.name} onClick={() => setColor(accent.name)} className={`accent-swatch ${color === accent.name ? 'accent-selected' : ''}`} style={{ backgroundColor: accent.value }} />)}</div></div><div className="course-form-actions"><button type="button" className="secondary-button" onClick={close}>Cancel</button><button type="submit" className="primary-button" disabled={!name.trim()}>Confirm & Save</button></div></form></section></div>;
}

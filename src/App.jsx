...
import { BookOpen, FileText, Folder, History, Moon, Settings2, Sparkles, Timer, Trash2, User, X, Zap } from 'lucide-react';
...
const INITIAL_COURSES = [{
  ...
  name: DEFAULT_COURSE.title,
  description: DEFAULT_COURSE.subtitle,
  color: 'sky',
  progress: DEFAULT_COURSE.progress,
  collections: [
...
export default function App() {
  ...
  const openCourse = (id) => { setSelectedCourseId(id); setPage('collections'); setMessage(''); };
  ...
  const addCourse = (event) => {
    ...
    setCourses((current) => [...current, { id: `course-${Date.now()}`, name, description: 'New course workspace', color: 'violet', progress: 0, collections: [] }]);
    setNewCourseName(''); setMessage('Course created.');
  };
  ...
  if (page === 'collections' && selectedCourse) return <CollectionsPage course={selectedCourse} newCollectionName={newCollectionName} setNewCollectionName={setNewCollectionName} addCollection={addCollection} onBack={() => setPage('workspace')} openCollection={openCollection} message={message} />;
  return <Dashboard courses={courses} totalNotes={totalNotes} performance={performance} setPerformanceMode={setPerformanceMode} newCourseName={newCourseName} setNewCourseName={setNewCourseName} addCourse={addCourse} openCourse={openCourse} deleteCourse={deleteCourse} action={action} setAction={setAction} message={message} onLogout={session ? () => supabase?.auth.signOut() : returnToLogin} />;
}

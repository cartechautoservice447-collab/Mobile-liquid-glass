...
import StudioWebGLBackground from './components/studio/StudioWebGLBackground.jsx';
import { useGlassBoxTracker } from './components/studio/useGlassBoxTracker.js';
import { DEFAULT_BG } from './data/backgroundTemplates.js';
...
export default function App() {
  ...
  const isModalOpen = Boolean(action || courseCreateOpen);
  const getBoxes = useGlassBoxTracker(isModalOpen);
  ...
  let screenContent;
  if (loading) screenContent = <main className="screen"><div className="glass-card loading-card">Loading your workspace…</div></main>;
  else if (!session && !skippedAuth) screenContent = <AuthScreen email={email} password={password} setEmail={setEmail} setPassword={setPassword} busy={busy} message={message} signIn={signIn} signUp={signUp} signInWithGoogle={signInWithGoogle} skipForNow={skipForNow} />;
  else if (page === 'courses') screenContent = <CourseFolderPage courses={courses} onBack={() => setPage('workspace')} onOpenCourse={openCourse} onAddCourse={openCourseCreator} onHome={() => setPage('workspace')} onCollections={() => courses[0]?.collections[0] && openCollection(courses[0].collections[0].id)} onNotes={() => courses[0] && (setSelectedCourseId(courses[0].id), setPage('course-notes'))} onMore={() => setAction('courses-more')} />;
  else if (page === 'course-workspace' && selectedCourse) screenContent = <CourseWorkspace course={selectedCourse} onBack={() => setPage('workspace')} onCourses={openCourseFolders} onCollections={() => setPage('collections')} onNotes={() => setPage('course-notes')} onStudySession={() => setAction('study')} onOverview={() => setAction('overview')} />;
  else if (page === 'editor' && selectedCourse && selectedCollection) screenContent = <EditorPage course={selectedCourse} collection={selectedCollection} note={selectedNote} content={editorContent} setContent={setEditorContent} onBack={async () => { await saveNote(); setPage('notes'); }} onSave={saveNote} message={message} />;
  else if (page === 'notes' && selectedCourse && selectedCollection) screenContent = <CollectionWorkspace course={selectedCourse} collection={selectedCollection} newNoteName={newNoteName} setNewNoteName={setNewNoteName} addNote={addNote} onBack={() => setPage('collections')} onSaveNote={saveCollectionNote} onDeleteNote={deleteCollectionNote} message={message} />;
  else if (page === 'course-notes' && selectedCourse) screenContent = <AllCourseNotesPage course={selectedCourse} onBack={() => setPage('course-workspace')} openCollection={openCollection} />;
  else if (page === 'collections' && selectedCourse) screenContent = <CollectionsPage course={selectedCourse} newCollectionName={newCollectionName} setNewCollectionName={setNewCollectionName} addCollection={addCollection} onBack={() => setPage('course-workspace')} openCollection={openCollection} message={message} />;
  else if (page === 'daily-planner') screenContent = <DailyPlannerPage session={session} onBack={() => setPage('workspace')} />;
  else screenContent = <Dashboard courses={courses} totalNotes={totalNotes} performance={performance} setPerformanceMode={setPerformanceMode} settings={settings} setSetting={setSetting} resetEngineSettings={resetEngineSettings} openCourseCreator={openCourseCreator} openCourseFolders={openCourseFolders} openCourse={openCourse} deleteCourse={deleteCourse} updateCourse={updateCourse} action={action} setAction={setAction} message={message} courseCreateOpen={courseCreateOpen} closeCourseCreator={closeCourseCreator} newCourseName={newCourseName} newCourseDescription={newCourseDescription} newCourseColor={newCourseColor} setNewCourseName={setNewCourseName} setNewCourseDescription={setNewCourseDescription} setNewCourseColor={setNewCourseColor} addCourse={addCourse} onLogout={session ? () => supabase?.auth.signOut() : returnToLogin} />;

  return (
    <>
      {settings.glassEngine === 'webgl' && (
        <StudioWebGLBackground
          currentBg={settings.webglBg || DEFAULT_BG}
          getBoxes={getBoxes}
          glParams={{
            thick: settings.webglThickness,
            bezel: settings.webglBezel,
            ior: settings.webglIOR,
            blur: settings.webglBlur,
            spec: settings.webglSpecular,
            tint: settings.webglTint,
            shadow: settings.webglShadow,
            dispersion: settings.webglDispersion,
          }}
